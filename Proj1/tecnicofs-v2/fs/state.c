...
static pthread_mutex_t i_table_lock;
...
static pthread_mutex_t data_table_lock;
...
static pthread_mutex_t of_table_lock;
...
void state_init() {
    ...
    pthread_mutex_init(&i_table_lock,NULL);
    ...
    pthread_mutex_init(&data_table_lock,NULL);
    ...
    pthread_mutex_init(&of_table_lock,NULL);
...
void state_destroy() { /* nothing to do */
    ...
    pthread_mutex_destroy(&i_table_lock);
    pthread_mutex_destroy(&data_table_lock);
    pthread_mutex_destroy(&of_table_lock);
}
...
int inode_create(inode_type n_type) {
    ...
    pthread_mutex_lock(&i_table_lock);
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                if (b == -1) {
                    ...
                    pthread_mutex_unlock(&i_table_lock);
                ...
                if (dir_entry == NULL) {
                    ...
                    //verificar se a criaçao da nova diretoria e bem feita

                    // porque no caso de nao ser o ponteiro devolve NULL

                    freeinode_ts[inumber] = FREE;
                    pthread_mutex_unlock(&i_table_lock);

                    // = FREE porque no inicio mudamos para TAKEN 
                ...
                for (int e=0;e<MAX_DATA_BLOCK_SIZE;e++){

                    inode_table[inumber].i_data[e] = -1;
...
int inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_node_type == T_DIRECTORY){
        ...
        /* Deletes each entry of the directory */

        for (size_t i = 0; i < MAX_DIR_ENTRIES; i++) {

            if (dir_entry[i].d_inumber!=-1){

                if (inode_delete(dir_entry[i].d_inumber) == -1){
                    pthread_mutex_unlock(&i_table_lock);
                    return -1;
                }
...
int data_block_alloc() {
    ...
    pthread_mutex_lock(&data_table_lock);
    ...
    for (int i = 0; i < DATA_BLOCKS; i++) {
        ...
        if (free_blocks[i] == FREE) {
            ...
            pthread_mutex_unlock(&data_table_lock);
            return i;
...
int add_to_open_file_table(int inumber, size_t offset) {
    ...
    pthread_mutex_lock(&of_table_lock);
    ...
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        ...
        if (free_open_file_entries[i] == FREE) {
            ...
            pthread_mutex_unlock(&of_table_lock);
